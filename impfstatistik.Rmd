...
update_dates <- read_xlsx(here::here(rki_tab_path), sheet = 1) %>% 
  rename(X1 = 1) %>% 
  filter(str_detect(X1, "Datenstand:")) %>% 
  transmute(date = str_extract(X1, "\\d{1,2}\\.\\d{1,2}\\.\\d{4}"),
         date = dmy(date),
         update_date_str = str_remove(X1, "Datenstand: ")) %>% 
  slice(1)
...
ggplot() +
  ...
  geom_text(aes(x = 1, y = vac_share, 
            ...
            hjust = 0, col = "grey30", nudge_x = 0.02, family = "Source Sans Pro", size = 12) +
...
p <- df %>%
  ...
  labs(title = "Anteil der Geimpften an der Gesamtbevölkerung", 
       x = NULL,  y = NULL) +
...
tidy_germany <- broom::tidy(shape_germany) %>% 
  ...
  mutate(vac_share = count_cumul / inhabitants)
...
state_centers <- tidy_germany %>% 
  ...
  mutate(label_abs = glue::glue("{NAME_1}<br><b style='color:{darken(main_color, 0.2)}'>
                                {count_cumul}</b>"),
         label_share = glue::glue("{NAME_1}<br><b style='color:{darken(main_color, 0.1)}'>
                                  {scales::percent(vac_share, accuracy = 0.01, decimal.mark = ',')}</b>")) %>% 
...
df2 %>%
  select(
    ...
    `Geimpfte (Bevölkerung)` = vac_share
  ) %>%
  datatable(
    style = "bootstrap4",
    rownames = FALSE,
    caption = glue::glue("Daten: RKI, Stand {update_dates$update_date_str}"),
    options = list(
      "pagelength" = 16,
      "paging" = FALSE,
  ...
  formatPercentage("Geimpfte (Bevölkerung)",
                   digits = 2,
                   dec.mark = ",")
